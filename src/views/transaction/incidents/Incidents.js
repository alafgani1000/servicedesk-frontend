...
const Incidents = () => {
  ...
  const EditDataAttachments = () => {
    if (incident !== undefined) {
      return (
        <CCard>
          <CCardHeader>Attachments</CCardHeader>
          <CCardBody>
            <CRow>
              <CCol>
      ...
      return <CRow>Data not found</CRow>;
    }
  };
          ...
          <CCardBody>
            <CForm>
              <CRow>
                <CCol>
                  <CFormGroup>
                    <CLabel htmlFor="incident" style={{ fontWeight: "bold" }}>
                      Incident/Problem:
                    </CLabel>
              ...
              <CRow>
                  ...
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge
                        ...
                        style={{ fontWeight: "bolder", fontSize: "12px" }}
                      >
                        {incident.userIncidents.name.charAt(0).toUpperCase() +
                          incident.userIncidents.name.slice(1)}{" "}
                        {moment(incident.createdAt).format("DD-MM-YYYY H:m:s")}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                </CCol>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge color="secondary" style={{ fontSize: "12px" }}>
                        {incident.ticket}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                ...
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge color="secondary" style={{ fontSize: "12px" }}>
                        {incident.sdate_ticket} {incident.stime_ticket}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                </CCol>
              ...
              <CRow>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge color="secondary" style={{ fontSize: "12px" }}>
                        {incident.edate_ticket} {incident.etime_ticket}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                </CCol>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge color="secondary" style={{ fontSize: "12px" }}>
                        {incident.resolve_date} {incident.resolve_time}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                </CCol>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      <CBadge color="secondary" style={{ fontSize: "12px" }}>
                        {incident.close_date} {incident.close_time}
                      </CBadge>
                    </CCardText>
                  </CFormGroup>
                </CCol>
              ...
              <CRow>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      {incident.categoryIncidents === null
                        ? ""
                        : incident.categoryIncidents.name}{" "}
                      (
                      {incident.categoryIncidents === null
                        ? ""
                        : incident.categoryIncidents.time_interval}{" "}
                      Jam)
                    </CCardText>
                  </CFormGroup>
                </CCol>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      {incident.teamIncidents === null
                        ? ""
                        : incident.teamIncidents.name}
                    </CCardText>
                  </CFormGroup>
                </CCol>
                <CCol>
                  <CFormGroup>
                    ...
                    <CCardText>
                      {incident.technicianIncident === null
                        ? ""
                        : incident.technicianIncident.name
                            .charAt(0)
                            .toUpperCase() +
                          incident.technicianIncident.name.slice(1)}
                    </CCardText>
                  </CFormGroup>
                </CCol>
              </CRow>
              ...
              <CRow>
                <CFormGroup>
                  <CCol>
                    ...
                    <CCardText>
                      {(incident.interval_resolve -
                        (incident.interval_resolve % 60)) /
                        60}{" "}
                      Hours {incident.interval_resolve % 60} Minutes
                    </CCardText>
                  </CCol>
                </CFormGroup>
              </CRow>
        ...
        return (
          <CTooltip content="Create ticket" placement="top-end">
            <CButton
              size="sm"
              onClick={() => {
                showModalCreateTicket(props.item);
              }}
              color="primary"
            >
  ...
  const Stage = (props) => {
    ...
    return <CBadge color={color}>{props.stage}</CBadge>;
  };
  ...
  const Category = (props) => {
    ...
    return (
      <CBadge style={{ backgroundColor: bgcolor, color: color }}>
        {props.category}
      </CBadge>
    );
  };
      ...
      .then(function (response) {
        ...
        Swal.fire({
          ...
          html:
            ...
            '</b><br><b>Dari <span class="badge bg-warning text-white">' +
            response.data.data.sdate_ticket +
            " " +
            response.data.data.stime_ticket +
            ' </span> Sampai <span class="badge bg-warning text-white">' +
            response.data.data.edate_ticket +
            " " +
            response.data.data.etime_ticket +
            "</span></b> ",
        });
        // socket io notif
        const socket = io(url);
      ...
      preConfirm: () => {
        Axs.patch(`api/incident/${item.id}/close`, {})
          .then((response) => {
            ...
            Swal.showValidationMessage(`Incident ${response.data.message}`);
          })
          .catch((error) => {
  ...
  return (
    <>
      <CRow>
        ...
        <CModal show={modalResolve} onClose={setModalResolve} size="">
        ...
        {/* modal create ticket */}
        <CModal show={modalCreateTicket} onClose={setModalCreateTicket}>
        ...
        {/* modal detail */}
        <CModal show={modalDetail} onClose={setModalDetail} size="lg">
        ...
        <CModal show={modalEdit} onClose={setModalEdit} size="lg">
          ...
          <CModalBody>
            <CForm onSubmit={handleUpdate} method="post">
              <CCard>
                <CCardHeader>Update Incident</CCardHeader>
                <CCardBody>
                  <CRow>
                    <CCol xs="12">
                      <CFormGroup>
        ...
        <CModal show={modalDelete} onClose={setModalDelete} size="sm">
          <CModalBody>Delete data ?</CModalBody>
          <CModalFooter>
            <CButton
              type="submit"
              color="danger"
        ...
        <CModal show={modalDelAttach} onClose={setModalDelAttach} size="sm">
          <CModalBody>Delete attachment ?</CModalBody>
          <CModalFooter>
            <CButton
              type="submit"
              color="danger"
                ...
                scopedSlots={{
                  ...
                  category: (item) => (
                    <td>
                      <Category
                        category={
                          item.categoryIncidents === null
                            ? ""
                            : item.categoryIncidents.name
                        }
                      />
                    </td>
                  ),
                  team: (item) => (
                    <td>
                      {item.teamIncidents === null
                        ? ""
                        : item.teamIncidents.name}
                    </td>
                  ),
                  user: (item) => <td>{item.userIncidents.name}</td>,
                  startDate: (item) => (
                    <td>
                      {item.sdate_ticket} {item.stime_ticket}
                    </td>
                  ),
                  endDate: (item) => (
                    <td>
                      {item.edate_ticket} {item.etime_ticket}
                    </td>
                  ),
                  createdAt: (item) => (
                    ...
                    <td>
                      <CButtonGroup>
                        ...
                        <ButtonDelete
                          item={item}
                          role={role}
                          stage={item.stageIncidents.text}
                        />
